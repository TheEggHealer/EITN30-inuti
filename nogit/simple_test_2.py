...
try:  # on Linux
  ...
  CSN_PIN = 10  # use CE0 on default bus (even faster than using any pin)
  CE_PIN = DigitalInOut(board.D27)  # using pin gpio22 (BCM numbering)

except ImportError:  # on CircuitPython only
  print('Det funkade inte D17')
...
def master(count=5):  # count = 5 will only transmit 5 packets
  ...
  while count:
    ...
    buffer = struct.pack("<b", payload[0])
...
def slave(timeout=6):
  ...
  while (time.monotonic() - start) < timeout:
    if nrf.available():
      ...
      # payload[0] = struct.unpack("<b", buffer[:4])[0]
      # print details about the received packet
      print(
        "Received {} bytes on pipe {}: {}".format(
          payload_size, pipe_number, str(buffer)
        )
      )
      start = time.monotonic()
